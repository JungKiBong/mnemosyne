...
class TestBatchIngestionAsyncAPI:
    ...
    def test_batch_async_lifecycle(self, client):
        ...
        resp = client.post('/api/ingest/batch/async', json={
            'graph_id': 'default',
        ...
        status_resp = client.get(f'/api/ingest/batch/status/{job_id}')
        ...
        final_data = client.get(f'/api/ingest/batch/status/{job_id}').get_json()

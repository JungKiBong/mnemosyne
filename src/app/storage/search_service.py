...
class SearchService:
    ...
    def _merge_results(
        ...
        Phase 7: Also applies salience-aware re-ranking and retrieval boost.
        """
        ...
        # Calculate hybrid scores (Phase 7: salience-weighted)
        scored = []
        for uid, item in all_items.items():
            v = v_scores.get(uid, 0.0)
            k = k_scores.get(uid, 0.0)
            base_score = self.VECTOR_WEIGHT * v + self.KEYWORD_WEIGHT * k

            # Phase 7: Salience-aware re-ranking
            # Items with higher salience get a slight boost (up to 15%)
            salience = item.get("salience")
            if salience is not None and isinstance(salience, (int, float)):
                salience_boost = salience * 0.15  # max 15% bonus
                combined = base_score * (1.0 + salience_boost)
            else:
                combined = base_score
        ...
        # Phase 7: Retrieval Boost — asynchronously boost salience for retrieved items
        self._async_retrieval_boost([r.get(key) for r in result if r.get(key)])

        return result

    def _async_retrieval_boost(self, uuids: List[str]):
        """
        Phase 7: Boost salience for items that were retrieved in search.
        Runs in a fire-and-forget manner to avoid blocking search response.
        """
        if not uuids:
            return
        try:
            ...
            t = threading.Thread(target=_boost, daemon=True)
            t.start()
        except Exception:
            pass  # Non-critical — boost is best-effort

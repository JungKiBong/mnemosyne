...
class GraphMemoryUpdater:
    ...
    def __init__(self, graph_id: str, storage: GraphStorage, observer_orchestrator=None):
        """
        Initialize GraphMemoryUpdater.

        Args:
            graph_id: GraphID
            storage: GraphStorage instance (injected)
            observer_orchestrator: Optional ObserverOrchestrator for ASMR cognitive extraction
        """
        self.graph_id = graph_id
        self.storage = storage
        self._observer = observer_orchestrator  # Phase 2: ASMR observer
        ...
        logger.info(f"GraphMemoryUpdater initialized: graph_id={graph_id}, batch_size={self.BATCH_SIZE}, observer={'enabled' if self._observer else 'disabled'}")

    def _get_platform_display_name(self, platform: str) -> str:
        return self.PLATFORM_DISPLAY_NAMES.get(platform.lower(), platform)

    def start(self):
        """Start background worker thread (and observers if attached)"""
        if self._running:
            return
        ...
        # Start ASMR Observer Agents (Phase 2)
        if self._observer:
            self._observer.start()
            logger.info(f"ASMR Observer Agents started for graph_id={self.graph_id}")

        logger.info(f"GraphMemoryUpdater started: graph_id={self.graph_id}")

    def stop(self):
        """Stop background worker thread (and observers if attached)"""
        self._running = False
        ...
        # Stop ASMR Observer Agents (Phase 2)
        if self._observer:
            self._observer.stop()
            logger.info(f"ASMR Observer Agents stopped for graph_id={self.graph_id}")
    ...
    def add_activity(self, activity: AgentActivity):
        """Add an agent activity to queue (and fan-out to observers)"""
        ...
        # Fan-out to ASMR Observer Agents (Phase 2)
        if self._observer:
            self._observer.observe(activity)
...
class GraphMemoryManager:
    ...
    def create_updater(
        ...
        Args:
            ...
            observer_orchestrator: Optional ObserverOrchestrator (Phase 2 ASMR)
        """
        with cls._lock:
            if simulation_id in cls._updaters:
                cls._updaters[simulation_id].stop()

            updater = GraphMemoryUpdater(graph_id, storage, observer_orchestrator)
            updater.start()

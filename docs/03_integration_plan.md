...
### 목표
Mnemosyne 메모리 시스템을 MCP (Model Context Protocol) Server로 래핑하여,
...
- [ ] **5.2** MCP Tool 정의 (핵심 5종)
  - `mnemosyne_search`: 지식 그래프 + SM 통합 검색
  - `mnemosyne_ingest`: 외부 데이터 소스 수집 (파일/URL/DB)
  - `mnemosyne_agent_profile`: 에이전트 프로필 조회
  - `mnemosyne_graph_query`: Cypher 쿼리 실행 (읽기 전용)
  - `mnemosyne_stream_control`: 스트림 수집 시작/중지
...
- [ ] **5.5** 멀티 에이전트 메모리 공유 프로토콜
  ...
  - 외부 AI → MCP Tool → Mnemosyne → 응답 흐름 E2E 테스트

### 완료 조건
- Claude/Gemini에서 MCP Tool로 `mnemosyne_search("Alice의 최근 행동은?")`을 호출하면 SearchResult가 반환됨
